...
- Booking flow chrome: use `src/components/patient/BookingFlowProgress/BookingFlowProgress.tsx` (segmented bar, steps 1–5 for service → psychologist → date/time → details → payment). Service selection uses a centered editorial hero + optional search + service cards with a **left pillar** accent when selected. Psychologist selection uses `bookingSplit`: sticky `bookingSidebar` (back + filters) and `bookingMainCanvas` with editorial header + card grid.
- Booking step cards (psychologist selection, date/time, review appointment): prefer **slightly darker surfaces** via `color-mix` on `$background-gray` / `$patient-primary` and **tighter padding** so the flow fits small viewports; headings use `$font-family-base` (sans), not marketing serif. Psychologist availability chips use `Badge` (`success` / `danger`) with module overrides (`.statusBadge`) for compact, higher-contrast fills.
